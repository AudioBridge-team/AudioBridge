...
## [1.3.2] - 09-10-2022
### Added:
* Предварительная оценка размера веса аудио перед скачиванием путём `F (bits) = t (sec) * bitrate (Kb / sec)`
* Появление кодов кастомных ошибок (`CustomError`) для обработки преднамеренной остановки работы потока (`AudioWorker`)
...
* Вынесение простых функций в ламбда-функции для улучшения читаемости кода
* Изменение содержания сообщений с процессом выполнения запроса
...
### Changed
* Доработа полноценная версия системы переключения между версиями (`prod.` и `dev.`) бота для разработчиков
* Изменена инфраструктура репозитория и структура хранения файлов на сервере
